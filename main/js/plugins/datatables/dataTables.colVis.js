...
if(this.s.showAll)
{nButton=this._fnDomShowAllButton();nButton.className+=" ColVis_ShowAll";this.dom.buttons.push(nButton);}
$(this.dom.collection).append(this.dom.buttons);},"_fnDomRestoreButton":function()
{var
that=this,dt=this.s.dt;return $('<li class="ColVis_Special '+(dt.bJUI?'ui-button ui-state-default':'')+'">'+this.s.restore+'</li>').click(function(e){for(var i=0,iLen=that.s.abOriginal.length;i<iLen;i++)
{that.s.dt.oInstance.fnSetColumnVis(i,that.s.abOriginal[i],false);}
that._fnAdjustOpenRows();that.s.dt.oInstance.fnAdjustColumnSizing(false);that.s.dt.oInstance.fnDraw(false);})[0];},"_fnDomShowAllButton":function()
{var
that=this,dt=this.s.dt;return $('<li class="ColVis_Special '+(dt.bJUI?'ui-button ui-state-default':'')+'">'+this.s.showAll+'</li>').click(function(e){for(var i=0,iLen=that.s.abOriginal.length;i<iLen;i++)
...
{ColVis.aInstances[i].fnRebuild();}}};ColVis.defaults={active:'click',buttonText:'Show / hide columns',aiExclude:[],bRestore:false,sRestore:'Restore original',bShowAll:false,sShowAll:'Show All',sAlign:'left',fnStateChange:null,iOverlayFade:500,fnLabel:null,bCssPosition:false,aoGroups:[],order:'column'};ColVis.aInstances=[];ColVis.prototype.CLASS="ColVis";ColVis.VERSION="1.1.0";ColVis.prototype.VERSION=ColVis.VERSION;if(typeof $.fn.dataTable=="function"&&typeof $.fn.dataTableExt.fnVersionCheck=="function"&&$.fn.dataTableExt.fnVersionCheck('1.7.0'))
...
$.fn.dataTable.ColVis=ColVis;$.fn.DataTable.ColVis=ColVis;return ColVis;};if(typeof define==='function'&&define.amd){define('datatables-colvis',['jquery','datatables'],factory);}
