						...
						<script language="javascript">
							document.title = "Report - Sales Forecast - Customer/Package | Eta Mifad";
						</script>
						<!-- /*******/ Page Name /*******/ -->
						...
						<div class="col-md-12">
							...
							<div class="box box-solid box-primary" id="first_div">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Sales Forecast Year <?php echo $_REQUEST['Year']; ?> - Customer/Package</h3>
									<a class="btn btn-lg btn-flat" href="<?php echo str_replace("package","unit",$_SERVER['REQUEST_URI']); ?>" target="_self">
										<i class="glyphicon glyphicon-transfer"></i>&ensp;<small><strong> to Unit</strong></small>
									</a>
								</div>
														...
														<?php 
															...
															if ($Result = mysqli_query($MySQLConnection,$Query1))
															{
															  ...
															  while ($row = $Result->fetch_assoc())
																{
																	echo 	'<tr>';
																	echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.$_REQUEST['Report'].'&Year='.$_REQUEST['Year'].'&CustomerNO='.$row['Customer_NO'].'&CustomerName='.$row['Customer_Name'].'&Company='.$row['Company'].'&Collapse">'.$row['Customer_NO'].'</a></td>';
																	echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.$_REQUEST['Report'].'&Year='.$_REQUEST['Year'].'&CustomerNO='.$row['Customer_NO'].'&CustomerName='.$row['Customer_Name'].'&Company='.$row['Company'].'&Collapse">'.$row['Customer_Name'].'</a></td>';
							...
							<?php if(isset($_REQUEST['CustomerNO']))
							{ 
								/********************************//********************************/
								/*Create a Query to Limit Users View by Sessions*/
								/********************************//********************************/
								if ((in_array($_SESSION['Department'], $Permission_SFCUserView)) && (in_array($_SESSION['Title'], $Permission_Admin_SV)))
								{
									$WHERE="WHERE sales_forecast.Year = ".$_REQUEST['Year']." AND sales_forecast.Customer_NO = ".$_REQUEST["CustomerNO"];
								}
								else
								{
									$WHERE="WHERE sales_forecast.Representative_NO = ".$_SESSION['ID']." AND sales_forecast.Year = ".$_REQUEST['Year']." AND sales_forecast.Customer_NO = ".$_REQUEST["CustomerNO"];
								}
								/********************************//********************************/
								// Perform queries 
								$Query2 =  "SELECT sales_forecast.ID
											...
											, sales_forecast.M_01	AS SUM_M_01
											, sales_forecast.M_02	AS SUM_M_02
											, sales_forecast.M_03	AS SUM_M_03
											, sales_forecast.M_04	AS SUM_M_04
											, sales_forecast.M_05	AS SUM_M_05
											, sales_forecast.M_06	AS SUM_M_06
											, sales_forecast.M_07	AS SUM_M_07
											, sales_forecast.M_08	AS SUM_M_08
											, sales_forecast.M_09	AS SUM_M_09
											, sales_forecast.M_10	AS SUM_M_10
											, sales_forecast.M_11	AS SUM_M_11
											, sales_forecast.M_12	AS SUM_M_12
											, sum(sales_forecast.M_01 * sales_forecast.Q_V_01)	AS SUM_M_V_01
											, sum(sales_forecast.M_02 * sales_forecast.Q_V_01)	AS SUM_M_V_02
											, sum(sales_forecast.M_03 * sales_forecast.Q_V_01)	AS SUM_M_V_03
											, sum(sales_forecast.M_04 * sales_forecast.Q_V_02)	AS SUM_M_V_04
							...
							<div class="box box-solid box-warning">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Forecasted in Details - Customer/Package</h3>
									<a class="btn btn-lg btn-flat" href="<?php echo str_replace("package","unit",$_SERVER['REQUEST_URI']); ?>" target="_self">
										<i class="glyphicon glyphicon-transfer"></i>&ensp;<small><strong> to Unit</strong></small>
									</a>
								</div>
								...
								<div class="box-body">
									<div class="callout">
											...
											<dt>Customer NO</dt>
											<dd><?php echo $_REQUEST["CustomerNO"]; ?></dd>
											<dt>Customer Name</dt>
											<dd><?php echo $_REQUEST["CustomerName"]; ?></dd>
											<dt>Company</dt>
											...
											<?php 
												...
												if ($Result = mysqli_query($MySQLConnection,$Query2))
												{
												  ...
												  while ($row = $Result->fetch_assoc())
													{
														echo 	'<tr>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-09","-08",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&Rep='.$row['Representative_NO'].'&RepName='.$row['Representative_Name'].'&Company='.$row['Company'].'">'.$row['Representative_Name'].'</a></td>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-09","-07",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&Item='.$row['Item_Code'].'&Item_Name='.$row['Item_Name'].'&Unit_Value='.$row['Unit_Value'].'&Unit='.$row['Unit'].'&Company='.$row['Company'].'">'.$row['Item_Code'].'</a></td>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-09","-07",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&Item='.$row['Item_Code'].'&Item_Name='.$row['Item_Name'].'&Unit_Value='.$row['Unit_Value'].'&Unit='.$row['Unit'].'&Company='.$row['Company'].'">'.$row['Item_Name'].'</a></td>';
							...
							<div class="box box-solid box-danger">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Product Forecast Year - Customer/Package</h3>
								</div>
								<div class="box-body chart-responsive">
									<?php $Chart = "./forms/chart/chart_customer_package.php"; ?>
