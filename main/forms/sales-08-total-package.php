						...
						<div class="col-md-12">
							...
							<div class="box box-solid box-warning">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Forecasted in Details - Representative/Total Package</h3>
									<a class="btn btn-lg btn-flat" href="<?php echo str_replace("package","unit",$_SERVER['REQUEST_URI']); ?>" target="_self">
										<i class="glyphicon glyphicon-transfer"></i>&ensp;<small><strong> to Unit</strong></small>
									</a>
								</div>
											...
											<?php 
												...
												if ($Result = mysqli_query($MySQLConnection,$Query2))
												{
												  ...
												  while ($row = $Result->fetch_assoc())
													{
														echo 	'<tr>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-08-total","-07",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&Item='.$row['Item_Code'].'&Item_Name='.$row['Item_Name'].'&Unit_Value='.$row['Unit_Value'].'&Unit='.$row['Unit'].'&Company='.$row['Company'].'">'.$row['Item_Code'].'</a></td>';
