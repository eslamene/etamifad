						...
						<script language="javascript">
							document.title = "Report - Sales Forecast - Representative/Package | Eta Mifad";
						</script>
						<!-- /*******/ Page Name /*******/ -->
						...
						<div class="col-md-12">
							...
							<div class="box box-solid box-primary" id="first_div">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Sales Forecast Year <?php echo $_REQUEST['Year']; ?> - Representative/Package</h3>
														...
														<?php 
															...
															if ($Result = mysqli_query($MySQLConnection,$Query1))
															{
															  ...
															  while ($row = $Result->fetch_assoc())
																{
																	echo 	'<tr>';
																	echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.$_REQUEST['Report'].'&Year='.$_REQUEST['Year'].'&Rep='.$row['Representative_NO'].'&RepName='.$row['Representative_Name'].'&Company='.$row['Company'].'">'.$row['Representative_Name'].'</a></td>';
							...
							{ 
								...
								if ((in_array($_SESSION['Department'], $Permission_SFCUserView)) && (in_array($_SESSION['Title'], $Permission_Admin_SV)))
								{
									$WHERE="WHERE sales_forecast.Year = ".$_REQUEST['Year']." AND sales_forecast.Representative_NO = ".$_REQUEST["Rep"];
								}
								else
								{
									$WHERE="WHERE sales_forecast.Representative_NO = ".$_SESSION['ID']." AND sales_forecast.Year = ".$_REQUEST['Year']." AND sales_forecast.Representative_NO = ".$_REQUEST["Rep"];
								}
								/********************************//********************************/
								// Perform queries 
							...
							<div class="box box-solid box-warning">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Forecasted in Details - Representative/Package</h3>
									<a class="btn btn-lg btn-flat" href="<?php echo str_replace("package","unit",$_SERVER['REQUEST_URI']); ?>" target="_self">
										<i class="glyphicon glyphicon-transfer"></i>&ensp;<small><strong> to Unit</strong></small>
									</a>
								</div>
								...
								<div class="box-body">
									<div class="callout">
											...
											<dd><?php echo $_REQUEST["RepName"]; ?></dd>
											<dt>Company</dt>
											...
											<?php 
												...
												if ($Result = mysqli_query($MySQLConnection,$Query2))
												{
												  ...
												  while ($row = $Result->fetch_assoc())
													{
														echo 	'<tr>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-08","-09",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&CustomerNO='.$row['Customer_NO'].'&CustomerName='.$row['Customer_Name'].'&Company='.$row['Company'].'">'.$row['Customer_NO'].'</a></td>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-08","-09",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&CustomerNO='.$row['Customer_NO'].'&CustomerName='.$row['Customer_Name'].'&Company='.$row['Company'].'">'.$row['Customer_Name'].'</a></td>';
														echo 		'<td>'.'<a href="'.$_SERVER['PHP_SELF'].'?Report='.str_replace("-08","-07",$_REQUEST['Report']).'&Year='.$_REQUEST['Year'].'&Item='.$row['Item_Code'].'&Item_Name='.$row['Item_Name'].'&Unit_Value='.$row['Unit_Value'].'&Unit='.$row['Unit'].'&Company='.$row['Company'].'">'.$row['Item_Code'].'</a></td>';
							...
							<div class="box box-solid box-danger">
								...
								<div class="box-header">
									...
									<h3 class="box-title">Product Forecast Year - Representative/Package</h3>
								</div>
								<div class="box-body chart-responsive">
									<?php $Chart = "./forms/chart/chart_rep_package.php"; ?>
